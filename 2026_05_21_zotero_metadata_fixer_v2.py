#!/usr/bin/env python3
"""
2026_05_21_zotero_metadata_fixer_v2.py

v2: skip ALL child items (parentItem is set), not just standalone attachments.
    Zotero only allows note/attachment/annotation as children, so trying to
    PATCH a child item to itemType=journalArticle returns 400 from the API.
...
def fix_item(zot, item, dry_run=True):
    ...
    # Attachments can never be journalArticle. Skip both standalone and
    # child attachments; v3 can promote standalone ones by creating parents.
    if itype == 'attachment':
        if parent:
            logging.info('  [skip] attachment (child of %s); cannot convert', parent)
        else:
            logging.info('  [skip] standalone attachment; needs parent (v3)')
        return False
